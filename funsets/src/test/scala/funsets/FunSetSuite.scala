...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains intersect elements of each set") {
    new TestSets {
      val u = union(s1,s2)
  ...
  test("diff contains elements in s not t"){
    new TestSets {
      val s = diff(s1,s2)
      assert(contains(s,1), "s not has 1")
      assert(!contains(s,2), "s has 2")
    }
  }
  ...
  test("filter"){
    new TestSets {
      ...
      assert(contains(f, 1),"not filter 1")
